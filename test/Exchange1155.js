...
describe("ExchangeMH", function () {
  ...
  beforeEach(async function () {
    ...
    await exchange.connect(operator).setAdminAddress(admin.address);
    await exchange
      .connect(operator)
    ...
    await exchange.connect(operator).setAdminFee(5);
    await exchange.connect(operator).setCreatorFee(10);
  });
  ...
  //     const adminOldBalance = await admin.getBalance();
  //     const creatorOldBalance = await creator.getBalance();
  //     const pricePerBox = ethers.utils.parseEther("1");
  //     const msgValue = ethers.utils.parseEther("1"); // price = 1 ether
  //     const options = { value: msgValue };
  //     await exchange
  //       .connect(user1)
